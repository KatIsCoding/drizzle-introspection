...
import type { AnyPgTable, PgEnum } from "drizzle-orm/pg-core";
...
import { getColumnCasing, sqlToStr } from "./serializer_utils";
...
export function buildArrayString(
	array: any[],
	incomingSqlType: string,
): string {
	const sqlType = incomingSqlType.split("[")[0];
	if (!sqlType) {
		throw new Error("No incoming SQL type");
	}
	const values = array
		.map((value) => {
	...
	for (const table of tables) {
		...
		primaryKeys.map((pk) => {
			...
			if (casing !== undefined) {
				for (let i = 0; i < originalColumnNames.length; i++) {
					const originalColName = originalColumnNames[i];
					if (!originalColName) {
						continue;
					}

					const newColName = columnNames[i];
					if (!newColName) {
						continue;
					}
					name = name.replace(originalColName, newColName);
				}
			}
		...
		const fks: ForeignKey[] = foreignKeys.map((fk) => {
			...
			if (casing !== undefined) {
				for (let i = 0; i < originalColumnsFrom.length; i++) {
					const originalColFrom = originalColumnsFrom[i];
					if (!originalColFrom) {
						continue;
					}

					const newColFrom = columnsFrom[i];
					if (!newColFrom) {
						continue;
					}
					name = name.replace(originalColFrom, newColFrom);
				}
				for (let i = 0; i < originalColumnsTo.length; i++) {
					const originalColTo = originalColumnsTo[i];

					const newColTo = columnsTo[i];

					if (!originalColTo || !newColTo) {
						continue;
					}
					name = name.replace(originalColTo, newColTo);
				}
			}
		...
		checks.forEach((check) => {
			...
			if (
				...
				checksInTable[`"${schema ?? "public"}"."${tableName}"`]?.push(
					checkName,
				);
			} else {
...
export const fromDatabase = async (
	...
	progressCallback?: (stage: any, count: number, status: any) => void,
...
WHERE 
  ...
  order by enum_schema, enum_name, sort_order;`,
	...
	const schemasForLinkedPoliciesInSchema = Object.values(
		tsSchema?.policies ?? {},
	).map((it) => it.schema!);
            ...
            AND    pg_class.relname = $2;`,
						...
						columnToReturn[columnName] = {
							...
							type:
								...
								!["vector", "geometry", "halfvec", "sparsevec", "bit"].includes(
									enumType,
								)
									? enumType
									: columnTypeMapped,
			...
			.map((value) => {
				if (
					...
					return JSON.stringify(
						JSON.stringify(JSON.parse(JSON.parse(value)), null, 0),
					);
				} else {
					return `\"${value}\"`;
				}
