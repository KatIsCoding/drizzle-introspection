...
export const mergedViewWithOption = viewWithOption
	.merge(matViewWithOption)
	.strict();
...
export const PgSquasher = {
	squashIdx: (idx: Index) => {
		...
		return `${idx.name};${idx.columns
			.map(
				(c) =>
					`${c.expression}--${c.isExpression}--${c.asc}--${c.nulls}--${c.opclass ? c.opclass : ""}`,
			)
			.join(
				",,",
	...
	unsquashIdx: (input: string): Index => {
		...
		if (!columnsString) {
			throw new Error("The columns strings wasn't found");
		}
	...
	squashIdxPush: (idx: Index) => {
		...
		return `${idx.name};${idx.columns
			...
			.join(",,")};${idx.isUnique};${idx.method};${JSON.stringify(idx.with)}`;
	},
	unsquashIdxPush: (input: string): Index => {
		const [name, columnsString, isUnique, method, idxWith] = input.split(";");

		if (!columnsString) {
			throw new Error("Missing column string from input");
		}
		...
		const result: Index = index.parse({
			...
			method,
			with:
				idxWith === "undefined" || idxWith === undefined
					? undefined
					: JSON.parse(idxWith),
		});
		return result;
	...
	unsquashPK: (pk: string): PrimaryKey => {
		const [cols, name] = pk.split(";");
		if (!cols || !name) {
			throw new Error("Columns or name are undefined");
		}
		return { name: name, columns: cols.split(",") };
	},
	...
	unsquashUnique: (unq: string): UniqueConstraint => {
		...
		if (!name || !columns) {
			throw new Error("No name or columns was found");
		}
		return {
			name,
	...
	unsquashFK: (input: string): ForeignKey => {
		...
		if (!columnsFromStr || !schemaTo || !columnsToStr) {
			throw new Error(
				"Either columns from str, to str or schema to wasn't found",
			);
		}
	...
	unsquashCheck: (input: string): CheckConstraint => {
		...
		if (!name || !value) {
			throw new Error("Name or value are undefined");
		}
