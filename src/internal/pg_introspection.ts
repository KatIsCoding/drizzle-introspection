...
import { indexName } from "./pg_serializer";
...
const intervalConfig = (str: string) => {
	...
	let statement = "{ ";
	statement += keys
		.map((it) => `${it}: ${json[it as keyof typeof json]}`)
		.join(", ");
	statement += " }";
	return statement;
};
...
export const relationsToTypeScriptForStudio = (
	...
	relations: Record<
		string,
		Relations<string, Record<string, Relation<string>>>
	>,
) => {
	...
	Object.values(relationsConfig.tables).forEach((table) => {
		...
		Object.values(relations).forEach((relation) => {
			...
			if (is(relation, One)) {
				...
				}, { fields: [${relation.config?.fields.map(
					(c) =>
						`${relationsConfig.tableNamesMap[getTableName(relation.sourceTable)]?.split(".")[1]}.${findColumnKey(
							relation.sourceTable,
							c.name,
						)}`,
				)}], references: [${relation.config?.references.map(
					(c) =>
						`${relationsConfig.tableNamesMap[getTableName(relation.referencedTable)]?.split(".")[1]}.${findColumnKey(
...
export const schemaToTypeScript = (
		...
		(res, it) => {
			...
			const columnImports = Object.values(it.columns)
				.map((col) => {
					...
					patched =
						patched === "double precision" ? "doublePrecision" : patched;
	...
	Object.values(schema.views).forEach((it) => {
		if (it.schema && it.schema !== "public" && it.schema !== "") {
			...
			it.materialized
				? imports.pg.push("pgMaterializedView")
				: imports.pg.push("pgView");
		}
...
const mapDefault = (
	...
	const isExpression =
		internals?.tables[tableName]?.columns[name]?.isDefaultAnExpression ?? false;
	const isArray = internals?.tables[tableName]?.columns[name]?.isArray ?? false;
	const lowered = type.toLowerCase().replace("[]", "");

	if (isArray) {
		return typeof defaultValue !== "undefined"
			? `.default(${buildArrayDefault(defaultValue, lowered)})`
			: "";
	}
...
const column = (
	...
	if (enumTypes.has(`${typeSchema}.${type.replace("[]", "")}`)) {
		let out = `${withCasing(name, casing)}: ${withCasing(paramNameFor(type.replace("[]", ""), typeSchema), casing)}(${dbColumnName(
			{ name, casing },
		)})`;
		return out;
	}
	...
	if (lowered.startsWith("bigserial")) {
		return `${withCasing(name, casing)}: bigserial(${dbColumnName({
			name,
			casing,
			withMode: true,
		})}{ mode: "bigint" })`;
	...
	if (lowered.startsWith("timestamp")) {
		...
		if (lowered.startsWith("timestamp(")) {
			...
			precision = Number(
				splitted[0]?.substring("timestamp(".length, splitted[0]?.length - 1),
			);
		}
	...
	if (lowered.startsWith("varchar")) {
		let out: string;
		if (lowered.length !== 7) {
			out = `${withCasing(name, casing)}: varchar(${dbColumnName({ name, casing, withMode: true })}{ length: ${lowered.substring(
				8,
				lowered.length - 1,
			)} })`;
		} else {
	...
	if (lowered.startsWith("geometry")) {
		...
		if (isGeoUnknown) {
			let unknown = `// TODO: failed to parse geometry type because found more than 2 options inside geometry function '${type}'\n// Introspect is currently supporting only type and srid options\n`;
	...
	if (lowered.startsWith("vector")) {
		let out: string;
		if (lowered.length !== 6) {
			out = `${withCasing(name, casing)}: vector(${dbColumnName({ name, casing, withMode: true })}{ dimensions: ${lowered.substring(
				7,
				lowered.length - 1,
			)} })`;
		} else {
	...
	if (lowered.startsWith("char")) {
		let out: string;
		if (lowered.length !== 4) {
			out = `${withCasing(name, casing)}: char(${dbColumnName({ name, casing, withMode: true })}{ length: ${lowered.substring(
				5,
				lowered.length - 1,
			)} })`;
		} else {
...
const createTableIndexes = (
	...
	idxs.forEach((it) => {
		...
		let idxKey =
			it.name.startsWith(tableName) && it.name !== tableName
		...
		statement += `.using("${it.method}", ${it.columns
			.map((it) => {
				if (it.isExpression) {
		...
		statement +=
			it.with && Object.keys(it.with).length > 0
...
const createTablePolicies = (
	...
	policies.forEach((it) => {
		...
		const mappedItTo = it.to?.map((v) => {
			return rolesNameToTsKey[v]
				? withCasing(rolesNameToTsKey[v], casing)
				: `"${v}"`;
		});
